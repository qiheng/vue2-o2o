    ...
    <div class="animsition shop-manage" v-cloak>
        ...
        <div class="shop-manage-panel white">
            <router-link href="javascript:;" @click.prevent="manageShop" class="block p10 clearfix shop-manage__info" :to="{name:'shopEditSetup'}">
                <div class="pull-left">
                    <img class="logo" width="40" height="40" :src="shopMangeData.shop.pic ? shopMangeData.shop.pic + '?x-oss-process=image/resize,m_fixed,h_100,w_100' : 'images/empty/default_shop_logo.png'" alt="" />
                </div>
            ...
            <div class="swiper-container shop-manage__notice">
                <ul class="shop-manage__notice1">
                    ...
                    <li v-if="shopMangeData.shop.license == 0 || shopMangeData.shop.license == 3">
                        <a href="javascript:;" @click.prevent="authShop">
                            <ins class="white">当前店铺未认证，7天后自动关闭，请提交认证</ins>
                        </a>
                    </li>
    ...
    created: function() {
        ...
        this.$axios.get(this.$api.manageIndex)
            .then(function(shopMangeData) {
                ...
                console.log(shopMangeData)
                if (window.sessionStorage.shopInfo == undefined || window.sessionStorage.shopInfo != shopMangeData) {
                    window.sessionStorage.shopInfo = JSON.stringify(shopMangeData);
                    store.dispatch('saveShopInfo', shopMangeData);
                }
                // 功能模块
                var modules = [
                    {
